<!-- Begin::SVG-->
<svg {{ $attributes->merge(['class' => '']) }}
     aria-hidden="true"
     focusable="false"
     data-prefix="fad"
     data-icon="car-battery"
     class="group-hover:text-green-400 text-gray-500 h-5 w-5"
     role="img"
     viewBox="0 0 512 512"
     xmlns="http://www.w3.org/2000/svg">
  <g class="fa-group">
    <path class="fa-secondary"
          fill="currentColor"
          d="M176 64h-96C71.13 64 64 71.13 64 80V128h128V80C192 71.13 184.9 64 176 64zM432 64h-96C327.1 64 320 71.13 320 80V128h128V80C448 71.13 440.9 64 432 64z"></path>
    <path class="fa-primary"
          fill="currentColor"
          d="M448 128H64C28.8 128 0 156.8 0 191.1v192C0 419.2 28.8 448 63.1 448H448c35.2 0 64-28.8 64-63.1V191.1C512 156.8 483.2 128 448 128zM176 272h-96C71.16 272 64 264.8 64 256s7.164-16 16-16h96C184.8 240 192 247.2 192 256S184.8 272 176 272zM432 272h-32v32c0 8.836-7.164 16-16 16s-16-7.164-16-16v-32h-32C327.2 272 320 264.8 320 256s7.164-16 16-16h32v-32C368 199.2 375.2 192 384 192s16 7.164 16 16v32h32C440.8 240 448 247.2 448 256S440.8 272 432 272z"></path>
  </g>
</svg>
<!-- End::SVG -->
